...
BEGIN directory specific "extendedInterface" AGENTS.md , other input may regard other hierarchical directories.

The extendedInterface project is a 'fork' derivative of 'ubiquitous_bash' using the shell script functions, compile, etc, of 'ubiquitous_bash' as a library. To understand how this project works, please also read the ./_lib/ubiquitous_bash/AGENTS.md file .


# Notable "extendedInterface" Subdirectories and Files

## _prog

Most bash shellcode specific to extendedInterface . Compiled into 'ubiquitous_bash.sh' .

## _lib/nsis/extIface.nsi

The build for the MSWindows installer. Because the installer build runs under UNIX/Linus, and because the installer itself early installs a 'ubcp' Cygwin environment, 'ubiquitous_bash' bash shell script functions are called for both the build process and to carry out some of the installation steps.
